...
export default function Dashboard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      console.warn("Error fetching latest scan:", err);
    }
    ...
    try {
      ...
      console.log("🔥 DATA FROM MONGODB:", sensorData);
      
      if (sensorData && sensorData.length > 0) {
        setDbData(sensorData);
      } else {
        console.log("⚠️ Python responded, but the data array was EMPTY []!");
      }
    } catch (err) {
  ...
  // Calculate the rolling window of the last 5 readings for the expanded view
...
const styles = StyleSheet.create({
  ...
  // STYLES FOR THE EXPANDED STREAM
